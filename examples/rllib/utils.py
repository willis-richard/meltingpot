...
class MeltingPotEnv(multi_agent_env.MultiAgentEnv):
  ...
  def setup(self, env_config: ConfigDict):
    ...
    # At the moment only reward exchange is implemented, nevertheless we create
    # a full reward transfer matrix that specifies the proportion of the row
    # player reward that they keep for themselves (the diagonal) and the
    # proportions that they transfer to each column player. For reward exchange,
    # we have a symmetrical matrix with the diagonals equal to the self-interest
  ...
  def step(
    ...
    # If reward exchange is used, then multiple the vector of player rewards by
    # the reward transfer matrix
